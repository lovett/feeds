...
const subscribe = require('../../dispatcher/feed/subscribe');
...
describe('feed-subscribe', function() {
    ...
    it('inserts to userFeeds table', function (done) {
        ...
        self.emitter.emit('feed-subscribe', self.userId, [{ id: 201, title: 'my title'}], (err) => {
            assert.ifError(err);
    ...
    it('skips user title when not specified', function (done) {
        ...
        self.emitter.emit('feed-subscribe', self.userId, [{ id: 201}], (err) => {
            assert.ifError(err);
    ...
    it('upserts when re-subscribing', function (done) {
        ...
        self.emitter.emit('feed-subscribe', self.userId, [{id: 200, title: 'my other title'}], (err) => {
            assert.ifError(err);
    ...
    it('handles empty list of feed ids', function (done) {
        ...
        self.emitter.emit('feed-subscribe', self.userId, [], (err) => {
            assert.ifError(err);
    ...
    it('handles non-array value for feed list', function (done) {
        ...
        self.emitter.emit('feed-subscribe', self.userId, null, (err) => {
            assert.ifError(err);
